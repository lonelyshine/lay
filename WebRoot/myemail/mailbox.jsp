                    ...
                    <div class="ibox-content mailbox-content">
                        <div class="file-manager">
                            ...
                            <ul class="folder-list m-b-md" style="padding: 0">
                                <li>
                                    <a href="mailbox.html"> <i class="fa fa-inbox "></i> 收件箱 <span class="label label-warning pull-right" id="notreademail"></span>
                                    </a>
                                </li>
                                <li>
                    ...
                    <div class="pull-right mail-search">
                        <div class="input-group">
                            ...
                            <div class="input-group-btn">
                                <button onclick="searchTitle()" class="btn btn-sm btn-primary">搜索 </button>
                            </div>
                        </div>
                    </div>
...
<!--     全局js
    ...
    <script type="text/javascript">
        ...
        function queryEmail(email,pageNumber,pageSize){
            var basepath = document.getElementById('basepath').value; 
            var url =  basepath + "/EamilList.do"; //请求的网址  
            var reqParams = {'email':email, 'pageNumber':pageNumber,'pageSize':pageSize};//请求数据
            $(function () {     
                  $.ajax({  
                        type:"POST",  
                        url:url,  
						                    ...
						                    if(dataList[i].EMAILSTATUS==1){
						                      /* var temp = "<tr class='read'> <td class='check-mail'><div class='icheckbox_square-green' style='position: relative;' onmouseover='mOver(this)' onmouseout='mOut(this)' onmousedown='mDown(this)'  ><input type='checkbox' class='i-checks' style='position: absolute; opacity: 0;'><ins class='iCheck-helper' style='position: absolute; top: 0%; left: 0%; display: block; width: 100%; height: 100%; margin: 0px; padding: 0px; border: 0px; opacity: 0; background: rgb(255, 255, 255);'></ins></div></td><td class='mail-ontact'><a href='mail_detail.html'>"+dataList[i].SENDER+"</a><span class='label label-warning pull-right'>验证邮件</span></td>"
 					                              ...
 					                              var temp = "<tr class='read'> <td class='check-mail'> <input id='emailid"+i+"' value='"+dataList[i].EMAILID+"' style='display:none'><input type='checkbox' id='check"+i+"' name='check1'  class='i-checks' ></td><td class='mail-ontact'><a href='mail_detail.jsp?emailid="+dataList[i].EMAILID+"'>"+dataList[i].SENDER+"</a></td><td><i class='fa fa fa-envelope-o btn-white'>已读</i></td>"
						                            +"<td class='mail-subject'><a href='mail_detail.jsp?emailid="+dataList[i].EMAILID+"'>"+dataList[i].TITLE+"</a></td><td class=''><i class='fa fa-paperclip'></i></td><td class='text-right mail-date'>"+dataList[i].THETIME+"</td></tr>";
  ...
  function mOver(ele){  //当鼠标悬停在这上面时
   ...
   function next(){
    ...
    function emailStatus(){ //按照邮件状态查询
       ...
       if(cla == 'fa fa-sort'){
          queryEmailByTerm('${userEmail}',page,pagenumber,'emailStatus','asc')
          document.getElementById("emailStatus").className = "fa fa-sort-up";
       }else if(cla == 'fa fa-sort-up'){
          queryEmailByTerm('${userEmail}',page,pagenumber,'emailStatus','desc')
    ...
    function titlename(){ //按照邮件标题查询
       ...
       if(cla == 'fa fa-sort'){
          queryEmailByTerm('${userEmail}',page,pagenumber,'title','asc')
          document.getElementById("titlename").className = "fa fa-sort-up";
       }else if(cla == 'fa fa-sort-up'){
          queryEmailByTerm('${userEmail}',page,pagenumber,'title','desc')
    ...
    function isupload(){ //按照邮件标题查询
       ...
       if(cla == 'fa fa-sort'){
          queryEmailByTerm('${userEmail}',page,pagenumber,'isupload','asc')
          document.getElementById("isupload").className = "fa fa-sort-up";
       }else if(cla == 'fa fa-sort-up'){
          queryEmailByTerm('${userEmail}',page,pagenumber,'isupload','desc')
    ...
    function thetime(){ //按照邮件标题查询
       ...
       if(cla == 'fa fa-sort'){
          queryEmailByTerm('${userEmail}',page,pagenumber,'thetime','asc')
          document.getElementById("thetime").className = "fa fa-sort-up";
       }else if(cla == 'fa fa-sort-up'){
          queryEmailByTerm('${userEmail}',page,pagenumber,'thetime','desc')
    ...
    * 通过条件查询邮件信息
    */
    function queryEmailByTerm(email,pageNumber,pageSize,term,sort){
            var basepath = document.getElementById('basepath').value; 
            var url =  basepath + "/EamilListByTerm.do"; //请求的网址  
            var reqParams = {'email':email, 'pageNumber':pageNumber,'pageSize':pageSize,'term':term,'sort':sort};//请求数据
            $(function () {     
                  $.ajax({  
                        type:"POST",  
                        url:url,  
        ...
        function pageChange(){ //当改变邮件每页展示数量后
          ...
          pagenumber = $('#emailnumber option:selected').text();//获取每页展示邮件信息的数量
          queryEmail(email,page,pagenumber);
          getTotal(document.getElementById('basepath').value);//刷新页数
          document.getElementById('searchTitle').value='';
        }
        
            
    //根据标题修改查询邮件数据
    function searchTitle(searchpage){
            var basepath = document.getElementById('basepath').value; 
            var url =  basepath + "/EamilListByTitle.do"; //请求的网址  
            ...
            var reqParams = {'email':email, 'pageNumber':temppage,'pageSize':pagenumber,'title':title};//请求数据
            $(function () {     
                  $.ajax({  
                        type:"POST",  
                        url:url,  
