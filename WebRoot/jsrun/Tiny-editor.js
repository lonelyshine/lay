...
TinyEditor.prototype = {
	...
	init : function(el,opt){
		...
		for(var i = 0,len=buffer.tabContainer.length;i<len;i++){
			buffer.tabContainer[i].style.height= (buffer.tabPlugin.offsetHeight-40)/2+'px';
		}
	...
	tab : function(index){
		for(var i = 0,len=buffer.tabControl.getElementsByTagName('li').length;i<len;i++ ){
			...
			buffer.tabControl.getElementsByTagName('li')[i].classList.add('active')
		}
		...
		buffer.tabControl.getElementsByTagName('li')[1].classList.add('active')
		buffer.tabControl.getElementsByTagName('li')[2].classList.add('active')
		buffer.tabControl.getElementsByTagName('li')[3].classList.add('active')
		*/
		
		buffer.tabContainer[index-1].style.zIndex='1';
		buffer.tabContainer[index-1].style.opacity='0';
		
		buffer.tabContainer[0].style.zIndex='1';
		buffer.tabContainer[0].style.opacity='1';
		buffer.tabContainer[1].style.zIndex='1';
		buffer.tabContainer[1].style.opacity='1';
		buffer.tabContainer[2].style.zIndex='1';
		buffer.tabContainer[2].style.opacity='1';
		buffer.tabContainer[3].style.zIndex='1';
		buffer.tabContainer[3].style.opacity='1';
		
		
		buffer.editorArr[index-1]&&buffer[buffer.editorArr[index-1]].focus()
		
		/*buffer.editorArr[0]&&buffer[buffer.editorArr[0]].focus()
		buffer.editorArr[1]&&buffer[buffer.editorArr[1]].focus()
		buffer.editorArr[2]&&buffer[buffer.editorArr[2]].focus()
		buffer.editorArr[3]&&buffer[buffer.editorArr[3]].focus()
        */
		...
		if(!!utils.dockMode){
			...
			*/
			...
			buffer.tabContainer[1].style.width=(buffer.tabPlugin.offsetWidth-utils.id('dockIframe').offsetWidth)+'px'
			buffer.tabContainer[2].style.width=(buffer.tabPlugin.offsetWidth-utils.id('dockIframe').offsetWidth)+'px'
			buffer.tabContainer[3].style.width=(buffer.tabPlugin.offsetWidth-utils.id('dockIframe').offsetWidth)+'px'
			*/
		}
	...
	dockMode : function(){
		if(!!utils.dockMode){   //-- 取消dock模式
			...
			document.getElementById('dockIframe').style.height=(buffer.tabPlugin.offsetHeight-40)/2+'px';
	...
	run:function(){
		...
		debugger;
		var html=buffer.htmlEditor.getValue()+"<script type='text/javascript' src='' ></script>";
		...
		var temphtml = iframe.document.getElementsByTagName('body')[0].getElementsByTagName('script');
		var length = temphtml.length;
		
		for(var i =0;i<length;i++){
		var oScript = document.createElement('script'); 
		oScript.type = 'text/javascript'; 
		oScript.async = true; 
		oScript.src = temphtml[i].src; 
		iframe.document.getElementsByTagName('body')[0].appendChild(oScript);
		//iframe.document.getElementsByTagName('body')[0].getElementsByTagName('script')[0].remove();
		}
